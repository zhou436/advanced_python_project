...
def constitutive_matrix(stress, seffmax, s0, nexp, E, v):
    ...
    C = (E/((1+v)*(1-2*v)))*np.array([[1-v, v, v, 0, 0, 0],
                                [v, 1-v, v, 0, 0, 0],
                                [v, v, 1-v, 0, 0, 0],
                                [0, 0, 0, 1/2*(1-2*v), 0, 0],
                                [0, 0, 0, 0, 1/2*(1-2*v), 0],
    ...
    seff = np.sqrt(((stress[:,0]-stress[:,1])**2 + stress[:,0]**2 + stress[:,1]**2 + 6*stress[:,2]**2)/2)
    for ii in range(4): # loop over all integration points
        if seff[ii] < seffmax[ii]:
            ...
            dev_stress[0] = stress[ii,0] - (stress[ii,0]+stress[ii,1])/3
            dev_stress[1] = stress[ii,1] - (stress[ii,0]+stress[ii,1])/3
            dev_stress[2] = 0 - (stress[ii,0]+stress[ii,1])/3
            dev_stress[3] = stress[ii,2]

            Cp = h*C@dev_stress@np.transpose(dev_stress)@C/((1+h*np.transpose(dev_stress)@C@dev_stress))
            Dep = C-Cp

        Dinv = np.linalg.inv(Dep)
        Dinv = np.delete(Dinv, [2,4,5], 0)
        Dinv = np.delete(Dinv, [2,4,5], 1)
